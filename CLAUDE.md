...
2. [../Lakbay.Docs/docs/02_BUILD_PLAN.md](../Lakbay.Docs/docs/02_BUILD_PLAN.md)
   ...
   `Lakbay.AvailabilityApi` — this repo has no phase of its own after Phase 0, it
...
operate. `Lakbay.Cms` and `Lakbay.AvailabilityApi` both implement this same
...
what keeps `Lakbay.Cms`'s write-model schema and `Lakbay.AvailabilityApi`'s
read-model schema from silently drifting apart (enforced by a schema-diff
CI check in `Lakbay.AvailabilityApi`) — both repos are real, permanently
...
`Lakbay.AvailabilityApi`'s resolvers and `Lakbay.Web`'s generated hooks — a
